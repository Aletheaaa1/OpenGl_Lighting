...
int main(void)
{
	...
	vao.AddBuffer(3, 3, GL_FLOAT, false, 8 * sizeof(float), 6 * sizeof(float));

	VertexArrayObject lighting_vao;
	lighting_vao.AddBuffer(2, 3, GL_FLOAT, false, 8 * sizeof(float), 0);

	Shader shader("./shader/LinghtingMapsPoint.shader");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Light
		LightPoint point_light(glm::vec3(0.0f, 0.0f, -0.0f), glm::vec3(glm::radians(0.0f), glm::radians(45.0f), 0.0f));

		float object_color[3] = { 1.0f, 1.0f, 1.0f };
		float ambient_color[3] = { 0.5f, 0.5f, 0.5f };
		float light_color[3] = { point_light.m_light_color.x, point_light.m_light_color.y, point_light.m_light_color.z };
		float light_position[3] = { point_light.m_light_position.x, point_light.m_light_position.y,
								   point_light.m_light_position.z };
		float light_direction[3] = { point_light.m_light_direction.x, point_light.m_light_direction.y,
									point_light.m_light_direction.z };
		...
		for (int i = 0; i < 10; i++)
		{
			...
			shader.SetUniform1f("lightPoint.constant", point_light.constant);
			shader.SetUniform1f("lightPoint.linear", point_light.linear);
			shader.SetUniform1f("lightPoint.quadratic", point_light.quadratic);
